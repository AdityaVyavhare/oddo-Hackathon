  ...
  const getDayActivities = (date) => {
    ...
    trip.cities?.forEach((cityStop) => {
      ...
      if (date >= cityStart && date <= cityEnd) {
        ...
        const cityActivities = (cityStop.activities || []).map(id => getActivityById(id)).filter(Boolean);
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-gray-500">
                      {dayActivities.length} {dayActivities.length === 1 ? 'city' : 'cities'}
                    </p>
                  </div>
                </div>
                  ...
                  {dayActivities.length > 0 ? (
                    dayActivities.map((item, idx) => (
                      <div key={idx} className="border-l-4 border-primary pl-4">
                        <h4 className="font-semibold text-primary mb-2">{item.city}</h4>
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Calendar: {trip.name}</h1>
